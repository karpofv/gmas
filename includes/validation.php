...
*
* Registrar datos sobre un documento.
*
...
* @license
* @version       v2.3.1
* @link          http://www.benjaminkeen.com/software/php_validation
* @see
* @since         15.10.2008
* @deprecated
* @file          doc_add.php
...
\*--------------------------------------------------------------------------------------------*/
function ValidarEntero($valor, $rangomaximo, $rangominimo)
{
    for ($i=0;$i<strlen($valor);$i++) {
        $ascii=ord($valor[$i]);
        if (intval($ascii) >=48 && intval($ascii) <=57) {
            continue;
        } else {
            return false;
        }
    ...
    if ((($rangominimo <>"")&&($valor<$rangominimo))||(($rangomaximo<>"")&&($valor>$rangomaximo))) {
        return false;
    }
    return true;
}
  ...
  for ($i=0; $i<count($rules); $i++) {
    ...
    $satisfies_if_conditions = true;
      ...
      if ($comparison == "equal" && $fields[$field_to_check] != $value_to_check) {
          ...
          break;
      } elseif ($comparison == "not_equal" && $fields[$field_to_check] == $value_to_check) {
          $satisfies_if_conditions = false;
          break;
      } else {
          array_shift($row);
    ...
    if (count($row) == 6) {        // valid_date
      $field_name2   = $row[2];
        ...
        $date_flag     = $row[4];
        $error_message = $row[5];
    } elseif (count($row) == 5) {     // reg_exp (WITH flags like g, i, m)
      $field_name2   = $row[2];
        $field_name3   = $row[3];
        $error_message = $row[4];
    } elseif (count($row) == 4) {     // same_as, custom_alpha, reg_exp (without flags like g, i, m)
      $field_name2   = $row[2];
        $error_message = $row[3];
    } else {
        $error_message = $row[2];
    ...
    switch ($requirement) {
      ...
      case "length":
        ...
        } elseif (preg_match("/length>=/", $length_requirements)) {
      ...
      case "range":
        ...
        } elseif (preg_match("/range>=/", $range_requirements)) {
            $comparison_rule = "greater_than_or_equal";
            $rule_string = preg_replace("/range>=/", "", $range_requirements);
        } elseif (preg_match("/range<=/", $range_requirements)) {
            $comparison_rule = "less_than_or_equal";
