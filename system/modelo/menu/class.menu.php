...
class Menu
{
    public function menuEmpUsuario($quien, $cedula, $pendiente='')
    {
        ...
        <li><a  onclick="$.ajax({ type: 'POST', url: 'accion.php', ajaxSend: $('#verContenido').html(cargando),
                      data: 'dmn=356',
        ...
        foreach ($filasMenu as $filasMenud) {
            ...
            if ($ccm!=$filasMenud[ConexMenuMaster]) {
                ...
                foreach ($filasMenuM as $filasMenudM) {
                    ?><li class="divider"></li>
                <li style="background: #333333;"><a><i class="glyph-icon icon-laptop"></i><span style="color: #FFFFFF;"><?php echo $filasMenudM[nombre_menu]; ?></span></a></li>
                <li class="divider"></li>
                <?php
